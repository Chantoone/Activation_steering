...
def show_attention_patterns(
    ...
    # ), f"translation_dataset must be an TranslationDataset {type(translation_dataset)}"
    prompts = translation_dataset.sentences
    ...
    for (layer, head) in heads:
        ...
        if precomputed_cache is None:
            ...
            with torch.no_grad():
                logits = model(translation_dataset.toks.long())
        else:
            cache = precomputed_cache
        attn_results = torch.zeros(
            size=(translation_dataset.N, translation_dataset.max_len, translation_dataset.max_len)
        )
        attn_results += -20

        for i, text in enumerate(prompts):
            # assert len(list(cache.items())) == 1 + int(mode == "val"), len(list(cache.items()))
            toks = translation_dataset.toks[i]  # model.tokenizer(text)["input_ids"]
...
def get_layer_mlp_in(
    model,
    translation_dataset,
    layer_idx
):
    mlp_in_hook_names = [f"blocks.{layer_idx}.hook_resid_mid"]
    
    fwd_cache = {}
    model.reset_hooks()
    model.cache_some(
        fwd_cache, lambda x: x in mlp_in_hook_names, suppress_warning=True
    )

    with torch.no_grad():
        _ = model(translation_dataset.toks.long()).detach()

    return model.blocks[layer_idx].ln2(fwd_cache[f"blocks.{layer_idx}.hook_resid_mid"])

def get_layer_mlp_out(
    model,
    translation_dataset,
    layer_idx
):
    mlp_in_hook_names = [f"blocks.{layer_idx}.hook_mlp_out"]
    ...
    return fwd_cache[f"blocks.{layer_idx}.hook_mlp_out"]



def get_translation_acc(
    model,
    translation_dataset,
):
    with torch.no_grad():
        logits = model(translation_dataset.toks.long()).detach()

    acc = 0
    for i in range(len(translation_dataset)):
        next_token_id = torch.argmax(logits[i, translation_dataset.word_idx["end"][i]])
        # print(f'[DEBUG] model generate: {model.tokenizer.decode([next_token_id])}\tGold Answer: {model.tokenizer.decode(translation_dataset.translation_tokenIDs[i])}')
        if next_token_id in translation_dataset.translation_tokenIDs[i]:
            acc += 1
    return acc / len(translation_dataset)


def get_translation_logits(
    model,
    translation_dataset,
    all=False,
    std=False,
    ...
    """
    # input_dict = {
    #     "input_ids": translation_dataset.toks.long(),
    #     "attention_mask": translation_dataset.attn_mask.long()
    # }
    # print(f'[DEBUG] translation_dataset.toks.device: {translation_dataset.toks.to("cuda").device}')
    with torch.no_grad():
        logits = model(translation_dataset.toks.long()).detach()

    #* 对每个example在sentence末尾END（预测下一个词的位置）取io_tokenIDs的logits值
    translation_logits = torch.zeros(len(translation_dataset), 1, 1).to(logits.device)
    for i in range(len(translation_dataset)):
        for translation_tokenID in translation_dataset.translation_tokenIDs[i]:
            translation_logits[i] += logits[
                i,
                translation_dataset.word_idx["end"][i],
                translation_tokenID,
            ]
        translation_logits[i] /= len(translation_dataset.translation_tokenIDs[i])

    return handle_all_and_std(translation_logits.squeeze(), all, std)
...
def batch_path_patching(
    ...
    """
    #khoi tao ket qua
    all_translation_logits = torch.zeros(len(D_orig)).to('cuda')
    for i in tqdm(range(0, len(D_new), batch_size)):
        batch_D_new = D_new[i: i+batch_size]
        batch_D_orig = D_orig[i: i+batch_size]
        # copy activation tu source cache vao hook, chi copy nhung position can thiet (vi neu copy het thi se bi loi kich thuoc khong khop)
        def patch_positions(z, source_act, hook, positions=["end"], verbose=False):
            for pos in positions:
                # print(f'[DEBUG] z.shape: {z.shape}')
                # print(f'[DEBUG] source_act.shape: {source_act.shape}')
                # print(f'[DEBUG] D_orig.N: {D_orig.N}')
                # print(f'[DEBUG] D_new.N: {D_new.N}')
                # print(f'[DEBUG] D_orig.word_idx[pos] length: {len(D_orig.word_idx[pos])}')
                # print(f'[DEBUG] D_new.word_idx[pos] length: {len(D_new.word_idx[pos])}')
                # print(f'[DEBUG] D_orig.word_idx[pos]: {D_orig.word_idx[pos]}')
                # print(f'[DEBUG] D_new.word_idx[pos]: {D_new.word_idx[pos]}')
                z[torch.arange(batch_D_orig.N), batch_D_orig.word_idx[pos]] = source_act[
                    torch.arange(batch_D_new.N), batch_D_new.word_idx[pos]
                ]
            return z
        ...
        with torch.no_grad():
            source_logits = model(batch_D_new.toks.long())
        ...
        model.cache_all(target_cache, suppress_warning=True)
        with torch.no_grad():
            target_logits = model(batch_D_orig.toks.long())
        ...
        for hook_name, head_idx in sender_hooks:
            ...
            model.add_hook(hook_name, hook)
        with torch.no_grad():
            receiver_logits = model(batch_D_orig.toks.long())
        ...
        for hook_name, head_idx in receiver_hooks:
            for pos in positions:
                if torch.allclose(
                    receiver_cache[hook_name][torch.arange(batch_D_orig.N), batch_D_orig.word_idx[pos]],
                    target_cache[hook_name][torch.arange(batch_D_orig.N), batch_D_orig.word_idx[pos]],
                ):
                    warnings.warn("Torch all close for {}".format(hook_name))
            hook = get_act_hook(
                partial(patch_positions, positions=positions),
        ...
        # Finish Path Patching Calculate the logits after patching
        cur_logits = get_translation_logits(model, batch_D_orig, all=True)
        all_translation_logits[i:i + batch_size] = cur_logits
    
    return all_translation_logits.mean().item()


# Component mapping for translation experiments (END position = translation_dataset.word_idx["end"]):
# - (f"blocks.{layer}.attn.hook_result", head_idx): attention head outputs, shape (batch, seq, n_heads, d_model); patch index head_idx at END.
# - (f"blocks.{layer}.hook_mlp_out", None): MLP outputs, shape (batch, seq, d_model); patch END position.
# - (f"blocks.{layer}.hook_resid_mid", None) / (f"blocks.{layer}.hook_resid_post", None): residual stream vectors, shape (batch, seq, d_model); patch END position.


def _get_position_indices(dataset, position_key="end", device=None):
    """Return tensor of token positions for each example."""
...
def get_activation(component, activations: Dict[str, torch.Tensor], position: torch.Tensor):
    """
    Extract a component activation at a specific position.

    component: Tuple[str, Optional[int]] -> (hook_name, head_idx)
    activations: cache dict from model.cache_some / cache_all
    position: tensor of shape (batch,) with indices to select
    """
    hook_name, head_idx = component
    act = activations[hook_name]
    batch_indices = torch.arange(act.shape[0], device=act.device)
    if act.dim() == 4:
        assert (
            head_idx is not None
        ), "Head index must be provided for attention result activations"
        return act[batch_indices, position, head_idx]
    if act.dim() == 3:
        return act[batch_indices, position]
...
def set_activation(
    ...
    """Set component activation at a specific position and return updated tensor."""
    hook_name, head_idx = component
    batch_indices = torch.arange(z.shape[0], device=z.device)
    if z.dim() == 4:
        assert (
            head_idx is not None
        ), f"Head index must be provided for attention activation {hook_name}"
        z[batch_indices, position, head_idx] = patched_value.to(z.device)
    elif z.dim() == 3:
        z[batch_indices, position] = patched_value.to(z.device)
    else:
        raise ValueError(f"Unsupported activation shape {z.shape} for {hook_name}")
    return z


def project_patch(a_plus: torch.Tensor, a_minus: torch.Tensor, W: torch.Tensor):
    """
    Equation (2): W W^T a_minus + (I - W W^T) a_plus
    """
    # ensure W is orthonormal
    W_orth, _ = torch.linalg.qr(W)
    proj = W_orth @ W_orth.T  # (d, d)
    identity = torch.eye(proj.shape[0], device=proj.device, dtype=proj.dtype)
    return torch.matmul(a_minus, proj.T) + torch.matmul(a_plus, (identity - proj).T)


def task_steering_subspace_identification(
    model,
    dataset_pos,
    dataset_neg,
    ...
    """
    Algorithm 1: identify task steering subspace for a single component.

    Returns: W_c with shape (d, rank_used) forming an orthonormal basis.
    """
    assert len(dataset_pos) == len(dataset_neg), "Contrastive datasets must align"
    hook_name, _ = component

    # cache activations for positive prompts
    pos_cache = {}
    model.reset_hooks()
    ...
    # cache activations for negative prompts
    neg_cache = {}
    ...
    pos_positions = _get_position_indices(dataset_pos, position_key, device=pos_cache[hook_name].device)
    neg_positions = _get_position_indices(dataset_neg, position_key, device=neg_cache[hook_name].device)

    a_pos = get_activation(component, pos_cache, pos_positions)  # (N, d)
    a_neg = get_activation(component, neg_cache, neg_positions)  # (N, d)
    deltas = a_pos - a_neg  # (N, d)
...
def _default_translation_metric(logits: torch.Tensor, dataset):
    """Compute translation logits at END position averaged over valid targets."""
    device = logits.device
    end_positions = _get_position_indices(dataset, "end", device=device)
    outputs = []
    for i in range(len(dataset)):
        tgt_ids = dataset.translation_tokenIDs[i]
        cur_logits = logits[i, end_positions[i], tgt_ids]
        outputs.append(cur_logits.mean())
...
def subspace_intervene_path_patching(
    ...
    subspace_dict: Dict[Tuple[str, Optional[int]], torch.Tensor],
    metric_fn: Optional[Callable[[torch.Tensor, Any], torch.Tensor]] = None,
    epsilon: float = 1e-8,
    receiver_hooks: Optional[List[Tuple[str, Optional[int]]]] = None,
    ...
    This mirrors the path_patching flow: only the path from the patched component
    to the specified receiver_hooks is changed, with other intermediate components
    frozen to the original (dataset_pos) activations.

    Returns a dict mapping component -> average relative change delta_c (% change).
    """
    assert len(dataset_pos) == len(dataset_neg), "Contrastive datasets must align"
    if metric_fn is None:
        metric_fn = _default_translation_metric
    ...
    # baseline on positive dataset
    model.reset_hooks()
    with torch.no_grad():
        logits_pos = model(dataset_pos.toks.long())
    y_orig = metric_fn(logits_pos, dataset_pos)

    pos_positions = {pos: _get_position_indices(dataset_pos, pos, device=logits_pos.device) for pos in positions}
    neg_positions = {pos: _get_position_indices(dataset_neg, pos, device=logits_pos.device) for pos in positions}
    ...
    for component in components:
        ...
        W_c = subspace_dict[component].to(logits_pos.device)

        # cache sender activations (negative dataset) for this component
        sender_cache = {}
        model.reset_hooks()
        ...
        # cache all activations on positive dataset for freezing
        target_cache = {}
        model.reset_hooks()
        model.cache_all(target_cache, suppress_warning=True)
        with torch.no_grad():
        ...
        def subspace_patch(z, hook, comp=component, W=W_c, src=sender_cache[hook_name], tgt=target_cache[hook_name]):
            for pos in positions:
                pos_idx_orig = pos_positions[pos]
                pos_idx_new = neg_positions[pos]
                a_plus = get_activation(comp, {hook_name: tgt}, pos_idx_orig)
                a_minus = get_activation(comp, {hook_name: src}, pos_idx_new)
                patched = project_patch(a_plus, a_minus, W)
                set_activation(comp, z, patched, pos_idx_orig)
            return z

        # First run to populate receiver_cache under patched setting (like path_patching C)
        receiver_cache = {}
        model.reset_hooks()
        ...
        # Final run D: reuse receiver_cache to restrict downstream paths
        model.reset_hooks()
        ...
        for r_name, r_head in receiver_hooks:
            ...
            def restore_positions(z, hook, alt_act=cached_act, pos_keys=positions):
                for pos in pos_keys:
                    pos_idx = dataset_pos.word_idx[pos].to(z.device)
                    patched_val = get_activation((r_name, r_head), {r_name: alt_act}, pos_idx)
                    set_activation((r_name, r_head), z, patched_val, pos_idx)
                return z

            model.add_hook(r_name, restore_positions)

        with torch.no_grad():
            logits_new = model(dataset_pos.toks.long())

        delta = (metric_fn(logits_new, dataset_pos) - y_orig) / (y_orig + epsilon)
...
def subspace_intervene_path_patching_batch(
    model,
    dataset_pos,
    dataset_neg,
    components: List[Tuple[str, Optional[int]]],
    ...
    """
    Batched variant of subspace_intervene_path_patching to avoid OOM.
    Accumulates average deltas over the full dataset using small batches.
    """
    if metric_fn is None:
        metric_fn = _default_translation_metric
    ...
    for start in range(0, len(dataset_pos), batch_size):
        ...
        batch_N = len(pos_batch)
        # reuse subspace bases; run per component to keep caches small
        batch_deltas = subspace_intervene_path_patching(
            model=model,
            dataset_pos=pos_batch,
            dataset_neg=neg_batch,
