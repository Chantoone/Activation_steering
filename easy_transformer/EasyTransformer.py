...
class EasyTransformer(HookedRootModule):
    ...
    def convert_hf_config(cls, hf_config, model_family):
        ...
        if model_family == "neo":
            ...
            cfg_dict = {
                "d_model": hf_config.hidden_size,          # Equivalent to n_embd in GPT-2
                "d_head": hf_config.hidden_size // hf_config.num_attention_heads,  # Derived from hidden_size and num_attention_heads
                "n_heads": hf_config.num_attention_heads,  # The number of attention heads
                "n_key_value_heads": hf_config.num_key_value_heads,  # The number of attention heads
                "d_mlp": hf_config.intermediate_size,      # Typically corresponds to n_embd * 4 in GPT-2 (feed-forward size)
                "n_layers": hf_config.num_hidden_layers,   # Number of transformer layers
                "n_ctx": hf_config.max_position_embeddings, # Maximum context length (context window size)
                "eps": hf_config.rms_norm_eps,       # Layer normalization epsilon
                "d_vocab": hf_config.vocab_size,           # Vocabulary size
                "positional_embedding_type": "rotary",
                "rotary_dim": hf_config.hidden_size // (hf_config.num_attention_heads * 2),
                "final_rms": True,
                "initializer_range": hf_config.initializer_range,
                "act_fn": hf_config.hidden_act,            # Activation function (usually "gelu" or "relu")
    ...
    def fold_layer_norm(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for l in range(self.cfg.n_layers):
            ...
            if self.cfg.model_family != "llama" and self.cfg.model_family != "mistral":
    ...
    def center_writing_weights(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for l in range(self.cfg.n_layers):
            ...
            if self.cfg.model_family in {"llama", "mistral", "qwen2"}:
                ...
                state_dict[f"blocks.{l}.mlp.b_down"] = (
                    state_dict[f"blocks.{l}.mlp.b_down"]
                    - state_dict[f"blocks.{l}.mlp.b_down"].mean()
                )
            else:
