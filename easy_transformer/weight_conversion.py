...
VALID_PRETRAINED_MODEL_NAMES = set(
    [
        ...
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/gpt2",
        "gpt2-small",
        "gpt2-medium",
        "gpt2-large",
        "gpt2-xl",
        ...
        "facebook/opt-125m",
        "facebook/opt-1.3b",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/opt-1.3b",
        "facebook/opt-2.7b",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/opt-2.7b",
        "facebook/opt-6.7b",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/opt-6.7b",
        "facebook/opt-13b",
        "facebook/opt-30b",
        "facebook/opt-66b",
        ...
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/Llama-2-7b",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/Llama-2-13b",
        "/online1/ycsc_chenkh/hitici_02/data/pretrained-models/Mistral-7B-v0.1",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/data/pretrained-models/ALMA-7B-Pretrain",
        "/home/export/base/ycsc_chenkh/hitici_02/online1/Easy-Transformer/exp_dir/precise_sft/saves/llama2-7b/full_sft/en-zh"
    ]
)
...
def convert_llama_weights(llama, cfg: EasyTransformerConfig):
    state_dict = {}
    ...
    for l in range(cfg.n_layers):
        ...
        # state_dict[f"blocks.{l}.ln1.b"] = torch.zeros(cfg.d_model)
        
        # Attention Projections: W_Q, W_K, W_V, and W_O
        ...
        W_Q = einops.rearrange(W_Q, "(i h) m->i m h", i=cfg.n_heads)
        W_K = einops.rearrange(W_K, "(i h) m->i m h", i=cfg.n_key_value_heads)
        W_V = einops.rearrange(W_V, "(i h) m->i m h", i=cfg.n_key_value_heads)
        state_dict[f"blocks.{l}.attn.W_Q"] = W_Q
        state_dict[f"blocks.{l}.attn.W_K"] = W_K
        state_dict[f"blocks.{l}.attn.W_V"] = W_V

        state_dict[f"blocks.{l}.attn.b_Q"] = torch.zeros(cfg.n_heads, cfg.d_head)
        state_dict[f"blocks.{l}.attn.b_K"] = torch.zeros(cfg.n_key_value_heads, cfg.d_head)
        state_dict[f"blocks.{l}.attn.b_V"] = torch.zeros(cfg.n_key_value_heads, cfg.d_head)
        ...
        # state_dict[f"blocks.{l}.ln2.b"] = torch.zeros(cfg.d_model)
        ...
        state_dict[f"blocks.{l}.mlp.W_up"] = llama.model.layers[l].mlp.up_proj.weight.T
        state_dict[f"blocks.{l}.mlp.b_up"] = torch.zeros(cfg.d_mlp)
        state_dict[f"blocks.{l}.mlp.W_down"] = llama.model.layers[l].mlp.down_proj.weight.T
        state_dict[f"blocks.{l}.mlp.b_down"] = torch.zeros(cfg.d_model)

    # Final Layer Norm
    state_dict["ln_final.w"] = llama.model.norm.weight
    # state_dict["ln_final.b"] = torch.zeros(cfg.d_model)
