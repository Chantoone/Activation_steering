...
def logit_diff(
    ...
    # print(f'[DEBUG] ioi_dataset.word_idx["end"] length: {len(ioi_dataset.word_idx["end"])}')
    # print(f'[DEBUG] ioi_dataset.io_tokenIDs length: {len(ioi_dataset.io_tokenIDs)}')
    ...
    #* 对每个example在sentence末尾END（预测下一个词的位置）取s_tokenIDs的logits值
    S_logits = logits[
        torch.arange(len(ioi_dataset)),
        ioi_dataset.word_idx["end"],
