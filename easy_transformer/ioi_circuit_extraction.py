...
def get_circuit_replacement_hook(
    ...
    # * heads_to_keep is not None others are None
    ...
    # print(f'[DEBUG] heads: {heads}')
    # print(f'[DEBUG] mlps: {mlps}')
...
CIRCUIT = {
    "name mover": [
        ...
        (9, 0),
        (11, 9),
        (10, 26), (1, 31), (31, 24), (23, 15), (14, 23), (6, 9),
        (23, 14), (15, 23), (19, 2)
    ],
    "negative": [(10, 7), (11, 10)],
...
def get_heads_circuit(ioi_dataset, excluded=[], mlp0=False, circuit=CIRCUIT):
    ...
    for circuit_class in circuit.keys():
        ...
        for head in circuit[circuit_class]:
            if head in excluded:
                continue
            heads_to_keep[head] = get_extracted_idx(["end"], ioi_dataset)
...
def do_circuit_extraction_mlp(
    ...
    print(f'mlps.keys(): {mlps.keys()}')
    print(f'heads.keys(): {heads.keys()}')
    # print(f'heads: {heads}')
    ...
    print(f'[DEBUG] AblationConfig: {config}')
    ...
    abl = EasyAblation(
        ...
        nodes=mlps.keys() if not cache_all_means else None,
    )
    model.reset_hooks()
    ...
    print(f'[DEBUG] heads_keys: {heads_keys}')
...
def do_circuit_extraction(
    ...
    abl = EasyAblation(
        ...
        mean_by_groups=True,  # TO CHECK CIRCUIT BY GROUPS
        groups=ioi_dataset.groups,
        nodes=heads.keys() if not cache_all_means else None,
    )
    model.reset_hooks()
