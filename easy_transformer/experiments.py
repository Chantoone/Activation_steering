...
class EasyAblation(EasyExperiment):
    ...
    def get_specific_mean(self, nodes):
        self.act_cache = {}
        self.model.reset_hooks()
        names = [f'blocks.{layer}.attn.hook_result' for (layer, _) in nodes]
        self.model.cache_some(self.act_cache, lambda x: x in names, suppress_warning=True)
        with torch.no_grad():
            self.model(self.cfg.mean_dataset)
        self.mean_cache = {}
        print(f'[DEBUG] act_cache.keys(): {self.act_cache.keys()}')
