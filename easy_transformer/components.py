...
class Attention(nn.Module):
    def __init__(
        ...
        self.W_K = nn.Parameter(
            torch.empty(self.cfg.n_key_value_heads, self.cfg.d_model, self.cfg.d_head)
        )
        ...
        self.num_key_value_groups = self.cfg.n_heads // self.cfg.n_key_value_heads
        
        self.b_Q = nn.Parameter(torch.zeros(self.cfg.n_heads, self.cfg.d_head))
        self.b_K = nn.Parameter(torch.zeros(self.cfg.n_key_value_heads, self.cfg.d_head))
        self.b_V = nn.Parameter(torch.zeros(self.cfg.n_key_value_heads, self.cfg.d_head))
            ...
            if self.cfg.model_family == 'llama' or self.cfg.model_family == 'mistral':
                ...
                if position_ids is None:
                    position_ids = torch.arange(
                        0, kv_seq_len, dtype=torch.long, device=q.device
                    ).unsqueeze(0)
                q, k = self.apply_rotary_pos_emb(q, k, cos, sin, position_ids)
                # repeat k/v heads if n_kv_heads < n_heads
                k = repeat_kv(k, self.num_key_value_groups)
                v = repeat_kv(v, self.num_key_value_groups)
                # Transpose q, k back to the original shape after applying the rotary embeddings
                q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
            else:
    ...
    def apply_rotary(self, x, past_kv_pos_offset=0):
        ...
        # print(f'[DEBUG] self.cfg.rotary_dim: {self.cfg.rotary_dim}')
        # print(f'[DEBUG] x_pos: {x_pos}')
        # print(f'[DEBUG] x_rot.shape: {x_rot.shape}')
        # print(f'[DEBUG] x_pass.shape: {x_pass.shape}')
        # print(f'[DEBUG] x_flip.shape: {x_flip.shape}')
        x_rotated = (
            x_rot
        ...
        # print(f'[DEBUG] x_rotated.shape: {x_rotated.shape}')
...
class LlamaMLP(nn.Module):
    def __init__(self, cfg: Union[Dict, EasyTransformerConfig]):
        ...
        self.W_up = nn.Parameter(torch.empty(self.cfg.d_model, self.cfg.d_mlp))
        self.b_up = nn.Parameter(torch.zeros(self.cfg.d_mlp))
        self.W_down = nn.Parameter(torch.empty(self.cfg.d_mlp, self.cfg.d_model))
        self.b_down = nn.Parameter(torch.zeros(self.cfg.d_model))

        self.hook_pre = HookPoint()  # [batch, pos, d_mlp]
        self.hook_up = HookPoint()  # [batch, pos, d_mlp]
        self.hook_post = HookPoint()  # [batch, pos, d_mlp]
    ...
    def forward(self, x):
        ...
        if not self.cfg.act_fn.endswith("_ln"):
            ...
            mid_act = self.hook_mid(self.act_fn(pre_act))  # [batch, pos, d_mlp]
            post_act = self.hook_post(self.ln(mid_act))
        up_proj = self.hook_up(
            einsum("batch pos d_model, d_model d_mlp -> batch pos d_mlp", x, self.W_up)
            + self.b_up
        )  # [batch, pos, d_mlp]
        mlp_out = (
            einsum(
